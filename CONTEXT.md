...
✅ 阶段 3 第 2 批：features/review/（12 文件，1 批完成）
   - features/review/             ReviewPage（独立页面，详见产品事实修正第 9 条）
...
└── lib/                                  (P2 阶段 4 第 1 批新增)
    ├── artifactSourceLabel.ts
    └── savedArtifactMeta.tsx
...
│   └── lib/                            (P2 阶段 4 第 1 批新增)
│       ├── artifacts/
│       │   ├── collectSavedArtifactsFromCloud.ts
│       │   ├── collectSavedArtifactsFromLocalHistory.ts
│       │   └── mergeArtifactLibraries.ts
│       └── mindMap/
│           ├── mindMapElkLayout.ts
│           ├── mindMapFlowAdapter.ts
│           ├── mindMapLabel.ts
│           ├── mindMapLayout.ts
│           └── mindMapScope.ts
├── sessionStart/
...
9. ReviewPage 不是 11 个工具的容器
   ────────────────────────────────────────────────────────
   旧描述："ReviewPage 是九宫格中枢，挂载 11 个学习工具"
   实际：ReviewPage.tsx 不 import 任何 tools/ 下的组件。
        它是独立的"学习产物库"页面，展示历史生成的产物
        （学习指南、思维导图、术语表等），让用户查看和管理。
   11 个工具实际全部由 App.tsx 直接挂载，每个由 App.tsx 的
   状态机控制弹层显隐。
   证据：REVIEW_PRE_MIGRATION_SCAN.md §2, §7.3
...
3. features/reader/lib/textUtils.ts 是 reader 多个子模块共用的"通用文本工具"，
