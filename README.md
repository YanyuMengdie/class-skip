...
- **1-2**：`utils/examChunkRetrieval.ts` 使用 **Okapi BM25**（自研轻量实现，**无额外 npm 依赖、无向量 API**）。中文为单字 + 二字 bigram 与英文整词混合分词；调试可加 `?debug=1` 在备考台底部试检索。
...
- **略读 / 智能导读（`SkimPanel`）**：**`chatWithSkimAdaptiveTutor`**，`systemInstruction` 为 **`utils/prompts.ts`** 的 STEM/HUMANITIES 长提示，与上者分离。
