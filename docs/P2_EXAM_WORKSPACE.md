...
- **`chatWithAdaptiveTutor`（备考苏格拉底等）**：`systemInstruction` 为上述教学法拼接 + **docType 辅助** + **模式**（非 `lib/prompts/systemPrompts.ts` 整段 STEM/HUMANITIES 长文）；可叠加 citations / chunk / KC / 支架等备考附录。
- **略读 / 智能导读**：`features/reader/skim/SkimPanel.tsx` 使用 **`chatWithSkimAdaptiveTutor`**，`systemInstruction` 来自 **`lib/prompts/systemPrompts.ts`** 的 **`STEM_SYSTEM_PROMPT` / `HUMANITIES_SYSTEM_PROMPT`**（按 `docType`），与备考路径分离。
