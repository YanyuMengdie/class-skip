...
### 问题 1：atom 数据从哪里来、存在哪里？

#### 存储位置

LogicAtom 数组**嵌在 KC 内部**（按 KC 分组）。[types.ts:171-188](types.ts#L171)：
...
这意味着前端 state 里**只有一处 truth source**：[App.tsx:216](App.tsx#L216) 的 `workspaceLsapContentMap`：

```ts
const [workspaceLsapContentMap, setWorkspaceLsapContentMap] = useState<LSAPContentMap | null>(null);
```

通过 `workspaceLsapContentMap.kcs[i].atoms` 访问。**没有独立的 `kcAtomsMap`、`atomList`、扁平数组等额外结构**——全仓 grep `kcAtomsMap | atomToKc | atomIdToKc | byAtomId | findKcForAtom | kcByAtom` 全部 0 命中。

每个 LogicAtom 自身也带 `kcId` 字段（自描述）。[types.ts:160-166](types.ts#L160)：

```ts
export interface LogicAtom {
  id: string;
  kcId: string;          // ← atom 自己记录所属 KC
  label: string;
  description: string;
}
```

所以**信息上是冗余的**：可以从外层 `kc.id` 拿，也可以从 `atom.kcId` 拿。

#### 加载生命周期

两条路径：

1. **从 localStorage bundle 恢复**（用户重新进入工作台 + 选定考试时）
   [App.tsx:378-384](App.tsx#L378)：
   ```ts
   const bundle = loadWorkspaceLsapBundle(key);
   if (bundle) {
   ...
   ```
   bundle 由 [features/exam/lib/examWorkspaceLsapKey.ts](features/exam/lib/examWorkspaceLsapKey.ts) 持久化。

2. **手动触发提取**（用户在工作台点"提取逻辑原子"）
   [App.tsx:1339-1492](App.tsx#L1339) 的 `handleExtractLogicAtoms`，调用 [services/geminiService.ts:2370](services/geminiService.ts#L2370) 的 `generateLogicAtomsForContentMap`，AI 出 atom 后回写到 `kc.atoms`：

   [App.tsx:1467-1468](App.tsx#L1467)：
   ```ts
...
atom id 在 [services/geminiService.ts:2452-2458](services/geminiService.ts#L2452) 生成：
...
📌 **关键观察**：atom id 的格式是 `atom-${kc.id}-${index}`——**kcId 已编码在 atomId 字符串里**。从 `atom-foo-2` 可以字符串解析出 KC id（如 `foo`）。但代码层面**没有任何**"用字符串切片解析 kcId"的逻辑——所有现存逻辑要么有外层 KC 上下文，要么用 `LogicAtom.kcId` 字段。

#### 结论

```
workspaceLsapContentMap (LSAPContentMap)
  └── kcs: LSAPKnowledgeComponent[]
       └── atoms?: LogicAtom[]
            └── { id, kcId, label, description }
```
...
```ts
// 假想代码（仓库未实现）
function getKcIdByAtomId(map: LSAPContentMap, atomId: string): string | null {
  for (const kc of map.kcs) {
    for (const atom of kc.atoms ?? []) {
      if (atom.id === atomId) return kc.id;
    }
  }
  return null;
}
...
时间复杂度 O(K × A)（K = KC 数，A = 平均 atom 数）。但因为：

1. **`atom.kcId` 字段存在**——单次拿到 LogicAtom 对象就能直接读 `atom.kcId`，免外层遍历
2. **id 字符串自带 kcId 编码**（`atom-${kc.id}-${index}`）——理论上可正则切；但项目里没人这么做

实际上**没有任何代码在做"从 atomId 反查 kcId"**——所有现存场景都从 KC 出发遍历它的 atoms（已知 kcId 的情况）。证据：全仓 grep `coveredAtomIds` 4 处命中：
- [services/geminiService.ts:2531-2557](services/geminiService.ts#L2531)（在 `analyzeKcUtteranceForAtoms` 内部，已知传入的 kc）
- [services/geminiService.ts:2566-2573](services/geminiService.ts#L2566)（同上的 wrapper）
- [features/exam/workspace/ExamWorkspaceSocraticChat.tsx:570-574](features/exam/workspace/ExamWorkspaceSocraticChat.tsx#L570)（已知 activeKc.id）
- 没有任何调用者拿到 atomId 后再去倒推 kcId
...
全仓 grep `getKcId | kcByAtom | atomToKc | atomIdToKc | byAtomId | findKcForAtom | kcOf` —— **0 命中**。

最接近的工具函数（都不是反查）：

- [features/exam/lib/examWorkspaceLsapKey.ts:58-71](features/exam/lib/examWorkspaceLsapKey.ts#L58) `mergeAtomCoverageForMap`：双层遍历 `kc.atoms`，目的是合并 coverage
- [features/exam/workspace/ExamWorkspaceSocraticChat.tsx:83-92](features/exam/workspace/ExamWorkspaceSocraticChat.tsx#L83) `atomProgressForKc`：传入 kc 后单层遍历该 kc 的 atoms 计 covered/total
- [features/exam/workspace/ExamWorkspacePage.tsx:109-118](features/exam/workspace/ExamWorkspacePage.tsx#L109) `atomCoverageCounts`：同上

代码层面：**没有任何"由 atomId 反查 kcId"的工具函数**。

---

### 问题 4：建立 atomId → kcId 反向索引的成本

#### 数据来源已具备

LogicAtom 自带 `kcId` 字段（[types.ts:162](types.ts#L162)），所以反向索引完全可以从 `workspaceLsapContentMap` 一次性 O(总 atom 数) 派生：

```ts
// 假想构建（仓库未实现）
function buildAtomToKcMap(map: LSAPContentMap | null): Record<string, string> {
  const idx: Record<string, string> = {};
  if (!map) return idx;
  for (const kc of map.kcs) {
    for (const atom of kc.atoms ?? []) {
      idx[atom.id] = kc.id;  // 或直接 atom.kcId
    }
  }
  return idx;
}
...
1. **App.tsx 父级 useMemo 派生**（与 `workspaceLsapContentMap` state 同步）
   - 派生：`useMemo(() => buildAtomToKcMap(workspaceLsapContentMap), [workspaceLsapContentMap])`
   - 加在 [App.tsx:216](App.tsx#L216) useState 后即可
   - 当 contentMap 变（提取原子、bundle 恢复、新生成图谱）时自动重建
   - 加 ~10 行（5 行函数 + 1 行 useMemo + 视情况向下传 props）

2. **生成时同步建**（atom 数据写入时）
   - 写入点：[App.tsx:1314, 1374, 1506](App.tsx#L1314)（setWorkspaceAtomCoverage 紧邻处）+ [services/geminiService.ts:2452-2458](services/geminiService.ts#L2452)（generateLogicAtomsForContentMap 内部）
   - 这种方式分散，不如 useMemo 派生集中

3. **作为 lazy 工具函数**（不存 state，每次用时算）
   - 反向索引派生自 contentMap，是纯函数，可放 [features/exam/lib/](features/exam/lib/) 下作 helper
   - 加 ~5 行函数定义

#### 内存成本量级

- 一个 atom id 字符串如 `"atom-kc_abc123-7"`，约 20-30 字节
- 一个 kc id 字符串约 10-20 字节
- 一个 Record entry 在 V8 中开销约 50-100 字节
- 假设 30 KC × 平均 5 atom = **150 entry**
- 总成本 **< 20 KB**——量级可忽略
...
| 文件 | 加行 |
|------|------|
| [features/exam/lib/examWorkspaceLsapKey.ts](features/exam/lib/examWorkspaceLsapKey.ts) | 加一个 `buildAtomToKcMap` 工具函数（~6 行） |
| [App.tsx](App.tsx) | 在 [App.tsx:216](App.tsx#L216) 后加一行 `useMemo` 派生（~3 行） |
| 各使用方 | 接收新 prop 或调用工具函数（视具体场景定） |

⚠️ **注意**：使用 `LogicAtom.kcId` 字段可能更轻——若手上已有 LogicAtom 对象，直接 `atom.kcId` 即可。是否真需要 atomId-only 的反查，取决于具体使用场景（代码层无法判断需求）。

---

### 问题 5：当前单 KC 模式下 atom 归属的处理逻辑

#### `mergeCoverageForKc` 调用前后完整代码

[features/exam/workspace/ExamWorkspaceSocraticChat.tsx:565-581](features/exam/workspace/ExamWorkspaceSocraticChat.tsx#L565)：
...
#### `analyzeKcUtteranceForAtoms` 服务侧

[services/geminiService.ts:2515-2563](services/geminiService.ts#L2515)：

```ts
export async function analyzeKcUtteranceForAtoms(
  mergedDocContent: string,
  kc: LSAPKnowledgeComponent,           // ← 单一 KC 入参
  userText: string
): Promise<{ coveredAtomIds: string[]; gapAtomIds: string[] }> {
  const atoms = kc.atoms ?? [];
  if (!atoms.length) return { coveredAtomIds: [], gapAtomIds: [] };
  const allowed = new Set(atoms.map((a) => a.id));   // ← 白名单 = 该 KC 的 atom id
  try {
...
...`;
    const response = await ai.models.generateContent({
      ...
      config: { ... responseSchema: { coveredAtomIds: STRING[], gapAtomIds: STRING[] } },
    });
    ...
    return {
      coveredAtomIds: filter(parsed.coveredAtomIds),                  // ← 白名单二次过滤
...
#### `mergeCoverageForKc` 函数本身

[features/exam/workspace/ExamWorkspaceSocraticChat.tsx:94-100](features/exam/workspace/ExamWorkspaceSocraticChat.tsx#L94)：
...
代码层有 **3 道防线**保证 `coveredAtomIds` 落到正确的 KC：

1. **AI 入参侧**（最强保证）
   `analyzeKcUtteranceForAtoms(mergedContent, activeKc, text)` 只把**单一 `activeKc` 的 atom 列表**喂给 AI（[geminiService.ts:2525-2528](services/geminiService.ts#L2525)）。AI 不知道其他 KC 的 atom，物理上**不可能**返回别的 KC 的 atom id。

2. **AI 输出过滤**（防 AI 幻觉）
   [geminiService.ts:2554-2555](services/geminiService.ts#L2554) 的 `allowed` Set 是 `activeKc.atoms` 的 id 集合，对 AI 返回值做白名单过滤——若 AI 编出不在列表里的 id 会被丢弃。

3. **客户端写入侧**
   [Socratic.tsx:573-575](features/exam/workspace/ExamWorkspaceSocraticChat.tsx#L573) 调 `mergeCoverageForKc(prev, activeKc.id, coveredAtomIds)`——**显式传入 `activeKc.id`** 作为归属 KC，atomId 们会写到 `cov[activeKc.id]` 这一行。

#### 显式断言？

**没有显式断言**（如 `assert(atomId.startsWith(\`atom-${kcId}-\`))` 之类）。等价的运行时保护通过"AI 入参限定 + 白名单过滤 + 显式传 kcId"达成——三道防线都依赖**外层调用上下文**确保 activeKc 的一致性，而不是验证 atomId 内部结构。

#### 关键含义（针对多选 KC 对话场景）

代码层观察到的客观事实：

- `analyzeKcUtteranceForAtoms` 签名只接受**单个 `kc: LSAPKnowledgeComponent`**——无法在一次调用里涵盖多 KC 的 atom 列表
- `mergeCoverageForKc` 只更新**单个 kcId 行**
- AI 返回的 `coveredAtomIds` 在当前协议下**没有 kcId 字段**——它是裸的 atom id 数组，"归属"靠"我只问了一个 KC"这个外层假设兜底
- 没有反向索引或反查函数可用——若让 AI 一次返回多个 KC 的 atom id 混合数组，前端目前**没有现成机制**把每个 atomId 归到正确的 KC
